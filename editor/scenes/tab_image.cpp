...
void tab_image::on_add_Button_clicked()
{
    ...
    snprintf(new_image.name, sizeof(new_image.name), "%s%d", "Show Image #", ScenesMediator::get_instance()->image_list.size()+1);
...
{
    if (data_loading) { return; }
    snprintf(ScenesMediator::get_instance()->image_list.at(ui->select_comboBox->currentIndex()).filename, sizeof(ScenesMediator::get_instance()->image_list.at(ui->select_comboBox->currentIndex()).filename), "%s", arg1.toStdString().c_str());
...
{
    if (data_loading) { return; }
    snprintf(ScenesMediator::get_instance()->image_list.at(ui->select_comboBox->currentIndex()).name, sizeof(ScenesMediator::get_instance()->image_list.at(ui->select_comboBox->currentIndex()).name), "%s", arg1.toStdString().c_str());
}
