...
void TabAnimation::on_add_pushButton_clicked()
{
    ...
    snprintf(new_animation.name, sizeof(new_animation.name), "%s%d", "Show Animation #", ScenesMediator::get_instance()->animation_list.size()+1);
...
{
    if (data_loading == true) { return; }
    snprintf(ScenesMediator::get_instance()->animation_list.at(ui->select_comboBox->currentIndex()).name, sizeof(ScenesMediator::get_instance()->animation_list.at(ui->select_comboBox->currentIndex()).name), "%s", arg1.toStdString().c_str());
}

void TabAnimation::on_filename_comboBox_currentIndexChanged(const QString &arg1)
{
    if (data_loading == true) { return; }
    snprintf(ScenesMediator::get_instance()->animation_list.at(ui->select_comboBox->currentIndex()).filename, sizeof(ScenesMediator::get_instance()->animation_list.at(ui->select_comboBox->currentIndex()).filename), "%s", arg1.toStdString().c_str());
