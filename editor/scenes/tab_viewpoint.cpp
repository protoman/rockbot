...
void tab_viewpoint::on_filename_comboBox_currentIndexChanged(const QString &arg1)
{
    ...
    snprintf(ScenesMediator::get_instance()->viewpoint_list.at(ui->select_comboBox->currentIndex()).filename, sizeof(ScenesMediator::get_instance()->viewpoint_list.at(ui->select_comboBox->currentIndex()).filename), "%s", arg1.toStdString().c_str());
...
void tab_viewpoint::on_add_Button_clicked()
{
    ...
    snprintf(new_viewpoint.name, sizeof(new_viewpoint.name), "%s%d", "Show Viewpoint #", ScenesMediator::get_instance()->viewpoint_list.size()+1);
...
{
    if (data_loading) { return; }
    snprintf(ScenesMediator::get_instance()->viewpoint_list.at(ui->select_comboBox->currentIndex()).name, sizeof(ScenesMediator::get_instance()->viewpoint_list.at(ui->select_comboBox->currentIndex()).name), "%s", arg1.toStdString().c_str());
}
