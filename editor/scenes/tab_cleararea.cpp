...
void TabClearArea::on_add_pushButton_clicked()
{
    ...
    snprintf(new_cleararea.name, sizeof(new_cleararea.name), "%s%d", "Clear Area #", ScenesMediator::get_instance()->cleararea_list.size()+1);
...
{
    if (data_loading == true) { return; }
    snprintf(ScenesMediator::get_instance()->cleararea_list.at(ui->select_comboBox->currentIndex()).name, sizeof(ScenesMediator::get_instance()->cleararea_list.at(ui->select_comboBox->currentIndex()).name), "%s", arg1.toStdString().c_str());
}
