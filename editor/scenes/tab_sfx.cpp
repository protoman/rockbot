...
void TabSfx::on_add_pushButton_clicked()
{
    ...
    snprintf(new_sfx.name, sizeof(new_sfx.name), "%s%d", "Play SFX #", ScenesMediator::get_instance()->playsfx_list.size()+1);
...
{
    if (data_loading == true) { return; }
    snprintf(ScenesMediator::get_instance()->playsfx_list.at(ui->select_comboBox->currentIndex()).name, sizeof(ScenesMediator::get_instance()->playsfx_list.at(ui->select_comboBox->currentIndex()).name), "%s", arg1.toStdString().c_str());
}

void TabSfx::on_filename_comboBox_currentIndexChanged(const QString &arg1)
{
    if (data_loading == true) { return; }
    snprintf(ScenesMediator::get_instance()->playsfx_list.at(ui->select_comboBox->currentIndex()).filename, sizeof(ScenesMediator::get_instance()->playsfx_list.at(ui->select_comboBox->currentIndex()).filename), "%s", arg1.toStdString().c_str());
