...
void TabText::on_AddButton_clicked()
{
    ...
    snprintf(new_text.name, sizeof(new_text.name), "%s%d", "Show Text #", ScenesMediator::get_instance()->text_list.size()+1);
...
void TabText::on_name_lineEdit_textChanged(const QString &arg1)
{
    ...
    snprintf(ScenesMediator::get_instance()->text_list.at(ui->select_comboBox->currentIndex()).name, sizeof(ScenesMediator::get_instance()->text_list.at(ui->select_comboBox->currentIndex()).name), "%s", arg1.toStdString().c_str());
}
