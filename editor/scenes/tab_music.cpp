...
void TabMusic::on_add_pushButton_clicked()
{
    ...
    snprintf(new_music.name, sizeof(new_music.name), "%s%d", "Play Music #", ScenesMediator::get_instance()->playmusic_list.size()+1);
...
{
    if (data_loading == true) { return; }
    snprintf(ScenesMediator::get_instance()->playmusic_list.at(ui->select_comboBox->currentIndex()).name, sizeof(ScenesMediator::get_instance()->playmusic_list.at(ui->select_comboBox->currentIndex()).name), "%s", arg1.toStdString().c_str());
}

void TabMusic::on_filename_comboBox_currentIndexChanged(const QString &arg1)
{
    if (data_loading == true) { return; }
    snprintf(ScenesMediator::get_instance()->playmusic_list.at(ui->select_comboBox->currentIndex()).filename, sizeof(ScenesMediator::get_instance()->playmusic_list.at(ui->select_comboBox->currentIndex()).filename), "%s", arg1.toStdString().c_str());
