...
void EditorArea::paintEvent(QPaintEvent *) {
    ...
    if (Mediator::get_instance()->show_background_color == true) {
        //painter.setPen(QColor(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].background_color.r, Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].background_color.g, Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].background_color.b, 255));
        painter.fillRect(QRectF(0.0, 0.0, MAP_W*TILESIZE*Mediator::get_instance()->zoom, MAP_H*TILESIZE*Mediator::get_instance()->zoom), QColor(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].background_color.r, Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].background_color.g, Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].background_color.b, 255));
	}
	// draw background1
    std::string bg1_filename(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].backgrounds[0].filename);
        ...
        if (filename.indexOf(".png") != -1) {
            ...
            if (!bg1_image.isNull()) {
                ...
                for (int k=0; k<max_repeat; k++) {
                    ...
                    QRectF pos_dest(QPoint(k*bg1_image.width()*Mediator::get_instance()->zoom, Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].backgrounds[0].adjust_y*Mediator::get_instance()->zoom), QSize(bg1_image.width()*Mediator::get_instance()->zoom, bg1_image.height()*Mediator::get_instance()->zoom));
	...
	// draw background2
    std::string bg2_filename(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].backgrounds[1].filename);
        ...
        if (filename.indexOf(".png") != -1) {
            ...
            if (!bg2_image.isNull()) {
                for (int k=0; k<((MAP_W*16)/bg2_image.width()*Mediator::get_instance()->zoom)+1; k++) {
                    ...
                    QRectF pos_dest(QPoint(k*bg2_image.width()*Mediator::get_instance()->zoom, Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].backgrounds[1].adjust_y*Mediator::get_instance()->zoom), QSize(bg2_image.width()*Mediator::get_instance()->zoom, bg2_image.height()*Mediator::get_instance()->zoom));
    ...
    for (i=0; i<MAP_W; i++) {
        for (j=0; j<MAP_H; j++) {
            ...
            if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked != 0 && Mediator::get_instance()->editTool == EDITMODE_LOCK) {
                    ...
                    if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked == 1) {
						terrainIcon = QString::fromUtf8(":/toolbar_icons/Lock"); // solid
                    } else if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked == 2) {
						terrainIcon = QString(":/toolbar_icons/stairs.png"); // stairs
                    } else if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked == 3) {
						terrainIcon = QString(":/toolbar_icons/object-order-lower.png"); // door
                    } else if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked == 4) {
						terrainIcon = QString(":/toolbar_icons/edit-delete.png"); // spikes
                    } else if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked == 5) {
						terrainIcon = QString(":/toolbar_icons/flag-blue.png"); // water
                    } else if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked == 6) {
						terrainIcon = QString(":/toolbar_icons/flag-green.png"); // ice
                    } else if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked == 7) {
						terrainIcon = QString(":/toolbar_icons/arrow-left.png"); // move left
                    } else if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked == 8) {
						terrainIcon = QString(":/toolbar_icons/arrow-right.png"); // move right
                    } else if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked == 9) {
						terrainIcon = QString(":/toolbar_icons/arrow-down.png"); // move right
                    } else if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked == 10) {
						terrainIcon = QString(":/toolbar_icons/system-switch-user.png"); // move right
                    } else if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].locked == 11) {
    ...
    if (Mediator::get_instance()->show_npcs_flag == true) {
        ...
        for (int i=0; i<FS_MAX_MAP_NPCS; i++) {
            ...
            int npc_id = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc;
            if (npc_id >= FS_GAME_MAX_NPCS || npc_id < 0) {
                ...
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc = -1;
                continue;
    //        } else {
    //            std::cout << "EditorArea::paintEvent #5.0.B - i[" << i << "], id_npc[" << npc_id << "] GOOD" << std::endl;
            }

            //std::cout << "EditorArea::paintEvent #5.0.C graphic_filename[" << game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].graphic_filename << "]" << std::endl;
            std::string filename = FILEPATH + "/images/sprites/enemies/" + Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].graphic_filename;
            if (filename.length() <= 0) {
                continue;
            }
            ...
            if (!temp_image.isNull()) {
                ...
                int total_w = Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].frame_size.width*Mediator::get_instance()->zoom;
                int total_h = Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].frame_size.height*Mediator::get_instance()->zoom;
                int sprite_adjust_x = Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].sprites_pos_bg.x;
                int sprite_adjust_y = Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].sprites_pos_bg.y;


                std::string npc_bg_file(Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].bg_graphic_filename);
                if (npc_bg_file.length() > 0) {
                    ...
                    if (!bg_image.isNull()) {
                        QRectF bg_target(QPoint(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.x*16*Mediator::get_instance()->zoom, Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.y*16*Mediator::get_instance()->zoom), QSize(total_w, total_h));
                        QRectF bg_source(QRectF(QPoint(0, 0), QSize(bg_image.width(), bg_image.height())));
                        painter.drawPixmap(bg_target, bg_image, bg_source);
                    }
                }

                QRectF target(QPoint((Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.x*16+sprite_adjust_x)*Mediator::get_instance()->zoom, (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.y*16+sprite_adjust_y)*Mediator::get_instance()->zoom), QSize(total_w, total_h));
                QRectF source;
                if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].direction != ANIM_DIRECTION_RIGHT || temp_image.height() <= Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].frame_size.height) {
                    source = QRectF(QPoint(0, 0), QSize(Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].frame_size.width, Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].frame_size.height));
                } else {
                    source = QRectF(QPoint(0, Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].frame_size.height), QSize(Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].frame_size.width, Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].frame_size.height));
                }
                if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].boss.id_npc == Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc) {
                    // transparent green rectangle
                    painter.setBrush(QColor(0, 255, 0, 180));
                    painter.drawRect(target);
                } else if (Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].is_boss) {
                    // transparent orange rectangle
                    painter.setBrush(QColor(255, 128, 35, 200));
                    painter.drawRect(target);
                } else if (Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].is_sub_boss) {
    ...
    if (Mediator::get_instance()->show_objects_flag == true) {
        ...
        for (int i=0; i<FS_MAX_MAP_OBJECTS; i++) {
            int obj_id = (int)Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].id_object;
            if (obj_id != -1) {
                ...
                if (temp_image.isNull()) {
                    ...
                    painter.drawRect(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.x*16*Mediator::get_instance()->zoom, Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.y*16*Mediator::get_instance()->zoom, 16*Mediator::get_instance()->zoom, 16*Mediator::get_instance()->zoom);
                } else {
                    int obj_type = Mediator::get_instance()->game_data.objects[obj_id].type;
                    int obj_direction = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].direction;
                    if (obj_type == OBJ_RAY_HORIZONTAL) {
                        ...
                        QRectF target(QPoint(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.x*TILESIZE*Mediator::get_instance()->zoom - (Mediator::get_instance()->game_data.objects[obj_id].size.width-TILESIZE)*Mediator::get_instance()->zoom, Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.y*16*Mediator::get_instance()->zoom), QSize(Mediator::get_instance()->game_data.objects[obj_id].size.width*Mediator::get_instance()->zoom, Mediator::get_instance()->game_data.objects[obj_id].size.height*Mediator::get_instance()->zoom));
                ...
                if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].map_dest != -1) {
                    ...
                    int dest_x = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.x*TILESIZE*Mediator::get_instance()->zoom + (obj_w/2)*Mediator::get_instance()->zoom - TILESIZE/2;
                    int dest_y = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.y*TILESIZE*Mediator::get_instance()->zoom + (obj_h/2)*Mediator::get_instance()->zoom - TILESIZE/2;
                    std::cout << "DRAW OBJECT TELEPORTER ORIGIN[" << i << "] - map: " << (int)Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].map_dest  << ", x: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.x << ", y: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.y << std::endl;
        ...
        for (int k=0; k<FS_STAGE_MAX_MAPS; k++) {
            for (int i=0; i<FS_MAX_MAP_OBJECTS; i++) {
                ...
                //std::cout << "OBJ[" << i << "].map_dest: " << (int)Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[k].map_objects[i].map_dest << ", currentMap: " << k << std::endl;
                if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[k].map_objects[i].map_dest == Mediator::get_instance()->currentMap) {
                    int obj_id = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[k].map_objects[i].id_object;
                    std::cout << "## EDITORAREA::paintEvent - teleporter_obj - x: " << (int)Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[k].map_objects[i].link_dest.x << ", y: " << (int)Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[k].map_objects[i].link_dest.y << std::endl;
                    if (Mediator::get_instance()->game_data.objects[obj_id].type == OBJ_FINAL_BOSS_TELEPORTER) {
                        painter.setBrush(QColor(160, 60, 60, 180));
                    } else {
                        painter.setBrush(QColor(60, 160, 60, 180));
                    }
                    painter.drawEllipse(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[k].map_objects[i].link_dest.x*16*Mediator::get_instance()->zoom, Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[k].map_objects[i].link_dest.y*16*Mediator::get_instance()->zoom, 16*Mediator::get_instance()->zoom, 16*Mediator::get_instance()->zoom);
                    // cyan border
                    painter.setBrush(Qt::NoBrush);
                    painter.setPen(QColor(0, 255, 255, 255));
                    painter.drawEllipse(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[k].map_objects[i].link_dest.x*16*Mediator::get_instance()->zoom, Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[k].map_objects[i].link_dest.y*16*Mediator::get_instance()->zoom, 16*Mediator::get_instance()->zoom, 16*Mediator::get_instance()->zoom);
                    painter.setPen(QColor(0, 0, 0, 255));
                    painter.drawText(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[k].map_objects[i].link_dest.x*16*Mediator::get_instance()->zoom + 3*Mediator::get_instance()->zoom, (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[k].map_objects[i].link_dest.y+1)*16*Mediator::get_instance()->zoom -2*Mediator::get_instance()->zoom, QString::number(i));
                }
            }
        }
    }
    ...
    if (Mediator::get_instance()->editMode == EDITMODE_NORMAL) {
        if (Mediator::get_instance()->editTool == EDITMODE_NORMAL) {
            if (Mediator::get_instance()->layerLevel == 1) {
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile1.x = Mediator::get_instance()->getPalleteX();
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile1.y = Mediator::get_instance()->getPalleteY();
            } else if (Mediator::get_instance()->layerLevel == 3) {
				printf(">> EditorArea::mousePressEvent - placing tile 3\n");
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile3.x = Mediator::get_instance()->getPalleteX();
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile3.y = Mediator::get_instance()->getPalleteY();
			}
        } else if (Mediator::get_instance()->editTool == EDITMODE_ERASER) {
            if (Mediator::get_instance()->layerLevel == 1) {
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile1.x = -1;
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile1.y = -1;
            } else if (Mediator::get_instance()->layerLevel == 3) {
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile3.x = -1;
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile3.y = -1;
			}
        } else if (Mediator::get_instance()->editTool == EDITMODE_LOCK) {
            if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].locked == 0) {
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].locked = Mediator::get_instance()->terrainType;
			} else {
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].locked = 0;
			}
        ...
        for (int i=0; i<MAX_MAP_NPC_N; i++) {
            if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc != -1 && Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.x == editor_selectedTileX && Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.y == editor_selectedTileY) {
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].boss.id_npc = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc;
                sprintf(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].boss.name, "%s", Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].name);
                break;
            }
        }
        ...
        for (int i=0; i<MAX_MAP_NPC_N; i++) {
            if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc != -1 && Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.x == editor_selectedTileX && Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.y == editor_selectedTileY) {
                ...
                Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].is_sub_boss = !Mediator::get_instance()->game_data.game_npcs[Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc].is_sub_boss;
                break;
            }
        }
...
/// @TODO: convert to new file-system ///
        ...
        for (int i=0; i<FS_MAX_MAP_OBJECTS; i++) {
            if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.x == -1 && Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.y == -1) {
                std::cout << "[OBJECT] reseting free-slot - i: " << i << ", id: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc << ", x: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.x << ", y: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.y << std::endl;
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc = -1;
			}
		}
        ...
        if (Mediator::get_instance()->editTool == EDITMODE_ERASER && found_npc != -1) {
            ...
            Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[found_npc].id_npc = -1;
        } else if (Mediator::get_instance()->editTool == EDITMODE_NORMAL && found_npc == -1 && Mediator::get_instance()->selectedNPC != -1) {
			// search for free slot
			for (int i=0; i<MAX_MAP_NPC_N; i++) {
                //std::cout << "searching for free-slot - i: " << i << ", id: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc << ", x: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.x << ", y: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.y << std::endl;
                if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc == -1) {

                    Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc = Mediator::get_instance()->selectedNPC;

                    std::cout << ">> added npc to map - stage[" << Mediator::get_instance()->currentStage << "], map[" << Mediator::get_instance()->currentMap << "], slot[" << i << "], id[" << Mediator::get_instance()->selectedNPC << "], set_value[" << (int)Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].id_npc << "]" << std::endl;

                    Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.x = editor_selectedTileX;
                    Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].start_point.y = editor_selectedTileY;
                    Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_npcs[i].direction = Mediator::get_instance()->npc_direction;
					repaint();
					return;
				}
			}
        ...
        for (int i=0; i<FS_MAX_MAP_OBJECTS; i++) {
            if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].id_object != -1 && Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.x == editor_selectedTileX && Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.y == editor_selectedTileY) {
				found_object = i;
				break;
			}
		}
		// clean old-format trash
        for (int i=0; i<FS_MAX_MAP_OBJECTS; i++) {
            if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.x == -1 && Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.y == -1) {
                std::cout << "reseting free-slot - i: " << i << ", id: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].id_object << ", x: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.x << ", y: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.y << std::endl;
                Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].id_object = -1;
			}
		}
        ...
        if (Mediator::get_instance()->editTool == EDITMODE_ERASER && found_object != -1) {
            ...
            Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[found_object].id_object = -1;
        } else if (Mediator::get_instance()->editTool == EDITMODE_NORMAL && found_object == -1 && Mediator::get_instance()->selectedNPC != -1) {
			// search for free slot
            for (int i=0; i<FS_MAX_MAP_OBJECTS; i++) {
                std::cout << "[OBJECT] searching for free-slot - i: " << i << ", id: " << (int)Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].id_object << ", x: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.x << ", y: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.y << std::endl;
                if (Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].id_object == -1) {
                    std::cout << "add object - slot: " << i << ", id: " << (int)Mediator::get_instance()->selectedNPC << std::endl;
                    Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].id_object = Mediator::get_instance()->selectedNPC;
                    Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.x = editor_selectedTileX;
                    Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].start_point.y = editor_selectedTileY;
                    Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].direction = Mediator::get_instance()->object_direction;
                    ...
                    if (Mediator::get_instance()->game_data.objects[Mediator::get_instance()->selectedNPC].type == OBJ_BOSS_TELEPORTER || Mediator::get_instance()->game_data.objects[Mediator::get_instance()->selectedNPC].type == OBJ_FINAL_BOSS_TELEPORTER) {
                        ...
                        Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].map_objects[i].map_dest = -1;
                    }
					repaint();
					return;
				}
...
void EditorArea::fill_area() {
    ...
    if (Mediator::get_instance()->layerLevel == 1) {
        originalX = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile1.x;
        originalY = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile1.y;
    } else if (Mediator::get_instance()->layerLevel == 3) {
        originalX = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile3.x;
        originalY = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile3.y;
	}
	// área a esquerda
	for (i=editor_selectedTileX; i>=0; i--) {
		...
		for (j=editor_selectedTileY; j>=0; j--) {
            if (Mediator::get_instance()->layerLevel == 1) {
                searchX = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].tile1.x;
                searchY = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].tile1.y;
            } else if (Mediator::get_instance()->layerLevel == 3) {
                searchX = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].tile3.x;
                searchY = Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].tiles[i][j].tile3.y;
			}
...
void EditorArea::saveGame(int game_n) {
    //std::cout << "EditorArea::saveGame ******** backgrounds[0].filename: " << Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].maps[Mediator::get_instance()->currentMap].backgrounds[0].filename << std::endl;
    ...
    for (int i=0; i<FS_MAX_STAGES; i++) {
        for (int j=0; j<FS_STAGE_MAX_MAPS; j++) {
            for (int k=0; k<FS_MAX_MAP_OBJECTS; k++) {
                Mediator::get_instance()->stage_data.stages[i].maps[j].map_objects[k].link_dest.x = -1;
                Mediator::get_instance()->stage_data.stages[i].maps[j].map_objects[k].link_dest.y = -1;
                Mediator::get_instance()->stage_data.stages[i].maps[j].map_objects[k].map_dest = -1;
            }
        }
    }
    */
...
void EditorArea::removeNpcFromMap(int posx, int posy) {
	...
	// empty npc list
    temp_list = Mediator::get_instance()->npc_map_list;
	while (temp_list) {
		if (temp_list->start_point.x == posx && temp_list->start_point.y == posy) {
			...
			if (item_ant != NULL) {
                ...
                Mediator::get_instance()->npc_map_list = temp_list;
			}
		} else {
			item_ant = temp_list;
...
void EditorArea::addObjectToMap(int posx, int posy) {
    ...
    Mediator::get_instance()->placeObject(posx, posy, new_obj);
