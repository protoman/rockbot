...
void convert_dialog_strings(v1_file_stage stage_v1, CURRENT_FILE_FORMAT::file_stage& stage_v2, short stage_id) {
    snprintf(stage_v2.dialog_face_graphics_filename, sizeof(stage_v2.dialog_face_graphics_filename), "%s", stage_v1.intro_dialog.face_graphics_filename);
...
void convert_stage_maps(int stage_id, v1_file_stage& stage_v1) {
    for (int i=0; i<V1_FS_STAGE_MAX_MAPS; i++) {
        for (int j=0; j<2; j++) {
            ...
            snprintf(maps_data[stage_id][i].backgrounds[j].filename, sizeof(maps_data[stage_id][i].backgrounds[j].filename), "%s", stage_v1.maps[i].backgrounds[j].filename);
...
void convert_stages_and_maps(v1_file_stages& stages) {
    for (int i=0; i<V1_FS_MAX_STAGES; i++) {
        ...
        snprintf(temp_v2.bgmusic_filename, sizeof(temp_v2.bgmusic_filename), "%s", temp_v1.bgmusic_filename);
        temp_v2.boss.id_npc = temp_v1.boss.id_npc;
        temp_v2.boss.id_weapon = temp_v1.boss.id_weapon;
        snprintf(temp_v2.boss.name, sizeof(temp_v2.boss.name), "%s", temp_v1.boss.name);
        ...
        snprintf(temp_v2.name, sizeof(temp_v2.name), "%s", temp_v1.name);
        snprintf(temp_v2.tileset_filename, sizeof(temp_v2.tileset_filename), "%s", "default.png");
...
void convert_ai_types(v1_file_game& game_v1) {
    for (int n=0; n<V1_FS_GAME_MAX_NPCS; n++) {
        ...
        snprintf(new_ai.name, sizeof(new_ai.name), "%s",  game_v1.ai_types[i].name);
...
void convert_game_npcs(v1_file_game& game_v1) {
    for (int i=0; i<V1_FS_GAME_MAX_NPCS; i++) {
        ...
        snprintf(new_enemy.bg_graphic_filename, sizeof(new_enemy.bg_graphic_filename), "%s", game_v1.game_npcs[i].bg_graphic_filename);
        ...
        snprintf(new_enemy.graphic_filename, sizeof(new_enemy.graphic_filename), "%s", game_v1.game_npcs[i].graphic_filename);
        ...
        snprintf(new_enemy.name, sizeof(new_enemy.name), "%s", game_v1.game_npcs[i].name);
...
void convert_game_objects(v1_file_game& game_v1) {
    for (int i=0; i<V1_FS_GAME_MAX_NPCS; i++) {
        ...
        snprintf(new_object.graphic_filename, sizeof(new_object.graphic_filename), "%s", game_v1.objects[i].graphic_filename);
        new_object.limit = game_v1.objects[i].limit;
        snprintf(new_object.name, sizeof(new_object.name), "%s", game_v1.objects[i].name);
...
void convert_projectiles(v1_file_game& game_v1) {
    for (int i=0; i<V1_FS_MAX_PROJECTILES; i++) {
        ...
        snprintf(new_projectile.graphic_filename, sizeof(new_projectile.graphic_filename), "%s", game_v1.projectiles[i].graphic_filename);
        ...
        snprintf(new_projectile.name, sizeof(new_projectile.name), "%s", game_v1.projectiles[i].name);
        snprintf(new_projectile.sfx_filename, sizeof(new_projectile.sfx_filename), "%s", game_v1.projectiles[i].sfx_filename);
...
void convert_game_players(v1_file_game& game_v1) {
    ...
    for (int j=0; j<FS_MAX_PLAYERS; j++) {
        ...
        snprintf(temp.face_filename, sizeof(temp.face_filename), "%s", game_v1.players[j].face_filename);
        temp.full_charged_projectile_id = game_v1.players[j].full_charged_projectile_id;
        snprintf(temp.graphic_filename, sizeof(temp.graphic_filename), "%s", game_v1.players[j].graphic_filename);
        temp.have_shield = game_v1.players[j].have_shield;
        temp.HP = game_v1.players[j].HP;
        temp.max_shots = game_v1.players[j].max_shots;
        snprintf(temp.name, sizeof(temp.name), "%s", game_v1.players[j].name);
...
void convert_game(v1_file_game& game_v1) {
    ...
    snprintf(game_data.name, sizeof(game_data.name), "%s", game_v1.name);
    ...
    for (int k=0; k<V1_MAX_STAGES; k++) {
        snprintf(game_data.stage_face_filename[k], sizeof(game_data.stage_face_filename[k]), "%s", game_v1.stage_face_filename[k]);
    }
    for (int k=0; k<V1_TROPHIES_MAX; k++) {
        game_data.trophies[k].condition = game_v1.trophies[k].condition;
        snprintf(game_data.trophies[k].filename, sizeof(game_data.trophies[k].filename), "%s", game_v1.trophies[k].filename);
        snprintf(game_data.trophies[k].name, sizeof(game_data.trophies[k].name), "%s", game_v1.trophies[k].name);
    }
    ...
    for (int k=0; k<V1_FS_MAX_WEAPONS; k++) {
        ...
        snprintf(game_data.weapons[k].name, sizeof(game_data.weapons[k].name), "%s", game_v1.weapons[k].name);
    }
...
void set_rockbot1_hardcoded() {
    ...
    snprintf(game_data.stages_face_name[0], sizeof(game_data.stages_face_name[0]), "%s", "INTRO");
    snprintf(game_data.stages_face_name[1], sizeof(game_data.stages_face_name[1]), "%s", "APE");
    snprintf(game_data.stages_face_name[2], sizeof(game_data.stages_face_name[2]), "%s", "DAISIE");
    snprintf(game_data.stages_face_name[3], sizeof(game_data.stages_face_name[3]), "%s", "SEAHORSE");
    snprintf(game_data.stages_face_name[4], sizeof(game_data.stages_face_name[4]), "%s", "MUMMY");
    snprintf(game_data.stages_face_name[5], sizeof(game_data.stages_face_name[5]), "%s", "MAGE");
    snprintf(game_data.stages_face_name[6], sizeof(game_data.stages_face_name[6]), "%s", "DYNAMITE");
    snprintf(game_data.stages_face_name[7], sizeof(game_data.stages_face_name[7]), "%s", "SPIKE");
    snprintf(game_data.stages_face_name[8], sizeof(game_data.stages_face_name[8]), "%s", "TECHNO");
    snprintf(game_data.stages_face_name[9], sizeof(game_data.stages_face_name[9]), "%s", "CASTLE");

    /// music (boss battle, final boss, game_over, got_weapon)
    snprintf(game_data.boss_music_filename, sizeof(game_data.boss_music_filename), "%s", "boss_battle.mod");
    snprintf(game_data.final_boss_music_filename, sizeof(game_data.final_boss_music_filename), "%s", "final_boss.mod");
    snprintf(game_data.got_weapon_music_filename, sizeof(game_data.got_weapon_music_filename), "%s", "got_weapon.mod");
    snprintf(game_data.game_over_music_filename, sizeof(game_data.game_over_music_filename), "%s", "OTHER.xm");
    snprintf(game_data.stage_select_music_filename, sizeof(game_data.stage_select_music_filename), "%s", "menu.mod");
    snprintf(game_data.game_start_screen_music_filename, sizeof(game_data.game_start_screen_music_filename), "%s", "opening.mod");
