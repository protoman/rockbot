...
void map_tab::on_stageTileset_comboBox_currentIndexChanged(const QString &arg1)
{
    ...
    if (arg1.length() == 0) { // reset to default
        ...
        snprintf(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].tileset_filename, sizeof(Mediator::get_instance()->stage_data.stages[Mediator::get_instance()->currentStage].tileset_filename), "%s", arg1.toStdString().c_str());
