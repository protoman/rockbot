...
void sceneShow::show_image(int n)
{
    if (image_scenes.size() <= n) {
        ...
        char number_str[20];
        snprintf(number_str, sizeof(number_str), "%zu", image_scenes.size());
...
void sceneShow::show_parallax(int n)
{
    if (parallax_scenes.size() <= n) {
        ...
        graphLib.show_debug_msg("EXIT #42.0");
        char number_str[20];
        snprintf(number_str, sizeof(number_str), "%zu", parallax_scenes.size());
...
void sceneShow::show_text(int n)
{
    if (text_list.size() <= n) {
        ...
        char number_str[20];
        snprintf(number_str, sizeof(number_str), "%zu", text_list.size());
...
void sceneShow::clear_area(int n)
{
    if (cleararea_list.size() <= n) {
        ...
        char number_str[20];
        snprintf(number_str, sizeof(number_str), "%zu", cleararea_list.size());
...
void sceneShow::play_sfx(int n)
{
    if (playsfx_list.size() <= n) {
        ...
        char number_str[20];
        snprintf(number_str, sizeof(number_str), "%zu", playsfx_list.size());
...
void sceneShow::play_music(int n)
{
    if (playmusic_list.size() <= n) {
        ...
        char number_str[20];
        snprintf(number_str, sizeof(number_str), "%zu", playmusic_list.size());
...
void sceneShow::show_viewpoint(int n)
{
    if (viewpoint_list.size() <= n) {
        ...
        char number_str[20];
        snprintf(number_str, sizeof(number_str), "%zu", viewpoint_list.size());
        exception_manager::throw_general_exception(std::string("sceneShow::play_music - Invalid list position."), std::string(number_str));
    }
    speed_x = 1;
    speed_y = 1;
